    ...
    try:
        ...
        for _ in range(num_suggestions):
            completion = client.chat.completions.create(
                model="llama3-8b-8192",
                messages=[
                    {"role": "system", "content": "You are a text-generation model. You get the words from the user and generate text starting with the words given by the user. For example, if the user says 'How', you can generate text like 'How are you?', 'How is your day going?', 'How old are you?', etc."},
                    {"role": "user", "content": input_text}
                ],
                max_tokens=12,  
                temperature=1.5,
